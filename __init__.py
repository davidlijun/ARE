...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(
    ["Performance Attribution", "Risk Report", "Scenario Stress Test", "Factor Attribution", "Efficient Frontier", "CURRENCY EXPOSURE & FX SENSITIVITY", "Rebalancing & Execution",
     "Relative Strength Signals", "Market Current Price"])
...
def fetch_premarket_and_gap(tickers):
    ...
    for t in tickers:
        try:
            ...
            # We filter for data before 9:30 AM EST (14:30 UTC)
            # Note: yfinance timestamps are usually UTC
            today_data = data['Adj Close'][t].dropna()
            
            # Pre-market price (last point before 9:30 AM)
            # If current time is pre-market, this is the 'Live' pre-market price
...
with tab9:
    ...
    **Objective:** Real-time monitoring of Price vs. Benchmark. 
    ...
    if not selected_tickers:
        ...
        try:
            live_data = fetch_live_prices(all_monitor_tickers)
    ...
    if st.button("Refresh Pre-Market/Gap Audit"):
        gap_df = fetch_premarket_and_gap(all_monitor_tickers)
    ...
    st.table(gap_df.style.map(highlight_gaps, subset=['Overnight Gap (%)']))
